...
User-invoked chat and inline rewriting send the request and the bounded context needed to answer it. When inline activation occurs in a recognized empty WhatsApp Web or Slack message editor, woof first reads a fresh, bounded slice of recent text from the nearest viable on-screen composer container and redacts it locally. WhatsApp message alignment may provide authorship evidence; Slack drafting proceeds only when visible labels explicitly identify authorship and otherwise fails closed. After that visible-context capture succeeds and the LLM request begins, woof cycles the literal temporary composer values `generating.`, `generating..`, and `generating...` through Accessibility-only writes and replaces the last confirmed value with the result. Marker updates never use the clipboard or keyboard. On failure or cancellation, woof removes an exact marker only while its controller still owns keyboard focus; otherwise it preserves the composer as-is and warns that temporary text may remain. These markers are never sent to OpenAI as conversation context or saved as inline-rewrite examples, but—like any transient visible application text—may be observed by ordinary background capture under the user's configured controls. The reply path inserts the result for review, never sends the message, and does not save the generated contextual reply as a local rewrite example. Selection and whole-draft rewrites do not trigger this fresh window capture. Realtime transcription sends microphone audio while dictation is active. Local Accessibility capture, full-text search, semantic embeddings, and vector search run without a remote embedding service.
...
The user can pause and resume capture and exclude applications. Capture uses Accessibility text, not screenshots, and refuses capture during secure input. Contextual reply drafting obeys the same pause and application-exclusion controls. Before reading the recursive text tree, it requires an exact supported Slack bundle or WhatsApp HTTPS host. It then requires a recognized message-composer label and one empty message editor; WhatsApp additionally requires exact-host web-document ancestry. The daemon verifies the foreground process, required window title, and exact focused AX window/element around the recursive read, using a positive native window number as an additional check when available. It does not persist the one-shot route result or save the generated reply as a local rewrite example. After insertion, the draft becomes ordinary visible application text, so background capture may store it under the configured controls. Removing the OpenAI API key prevents later operations from starting with that key; a scheduled generation run or user operation already in progress may finish.
