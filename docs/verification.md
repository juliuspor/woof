...
6. Add and remove an OpenAI key, then exercise chat, inline rewriting, and Realtime transcription without exposing the key. In WhatsApp Web with recent incoming and outgoing messages visible, focus a recognized empty message editor and double-tap the rewrite modifier. The composer must remain empty while woof captures and validates the recent visible conversation. Once the LLM request is in flight, the real composer must begin cycling `generating.`, `generating..`, and `generating...` in order through Accessibility-only writes, without clipboard or keyboard activity, then replace the exact current marker with the reply, reread the exact inserted draft, and never send. Simulate a slow response and multiple complete marker cycles. Cancel and force an API error while the edit controller remains focused; the exact marker must be removed. Type into the composer, focus another woof window, change focus, or switch chats during generation; woof must stop without overwriting or clearing user text. When safe cleanup cannot be proven, woof must warn that temporary text may remain rather than racing the composer. Force Secure Input or input failure after fallback delivery begins; woof must instruct the user to inspect the composer and must not claim the chat was untouched. Repeat in Slack with explicit visible authorship labels, then without them; the latter must report insufficient context. Also repeat with capture paused, the app excluded, a deceptive or unsupported domain/app, a WhatsApp editor outside the exact-host web document, an unlabeled text area, a Slack canvas, an empty search or single-line text field, a switch between two same-title windows, a changed tab/window or focused element during capture, an ignored marker or final write, no visible messages, a non-empty draft, and selected text; contextual reply must fail closed while draft and selection rewriting retain their original modes and do not perform a fresh window capture.
